...
VM_PC_NAME="CSV_PaperCut_Exploit_Demo"
VM_PC_IP="192.168.100.50"
VM_PC_SOURCE_VDI="files/CSV_PaperCut_Exploit_Demo.vdi" 
VM_PC_DIR="$HOME/VirtualBox VMs/$VM_PC_NAME"
VM_PC_DISK_PATH="$VM_PC_DIR/$VM_PC_NAME.vdi"

VM_KALI_NAME="CSV_Kali_Demo"
VM_KALI_IP="192.168.100.60"
VM_KALI_SOURCE_VDI="files/CSV_Kali_Demo.vdi" 
VM_KALI_DIR="$HOME/VirtualBox VMs/$VM_KALI_NAME"
VM_KALI_DISK_PATH="$VM_KALI_DIR/$VM_KALI_NAME.vdi"
...
init() {
	if [ ! -d "files" ] || [ ! -f "$VM_PC_SOURCE_VDI" ] || [ ! -f "files/pcng-setup-19.2.7.62200-linux-x64.sh" ]; then
...
provision_vm() {
	...
	#----------------------------------------------------
	ubuntu_vm() {
		VBoxManage createvm --name "$VM_PC_NAME" --ostype "Ubuntu_64" --register --basefolder "$HOME/VirtualBox VMs"

		echo "Copying VDI to VM directory..."
		cp "$VM_PC_SOURCE_VDI" "$VM_PC_DISK_PATH"

		# Configure Hardware
		VBoxManage modifyvm "$VM_PC_NAME" --graphicscontroller vmsvga --vram 128
		VBoxManage modifyvm "$VM_PC_NAME" --cpus 2 --memory 4096
		VBoxManage modifyvm "$VM_PC_NAME" --nic1 nat
		VBoxManage modifyvm "$VM_PC_NAME" --nic2 hostonly --hostonlyadapter2 $VBOXNET_INTERFACE
		VBoxManage modifyvm "$VM_PC_NAME" --natpf1 "guestssh,tcp,,2222,,22"
		VBoxManage modifyvm "$VM_PC_NAME" --natpf1 "papercut,tcp,,8191,,8191"
		...
		# Storage Controllers
		VBoxManage storagectl "$VM_PC_NAME" --name "SATA Controller" --add sata --controller IntelAhci

		# Attach the EXISTING VDI
		VBoxManage storageattach "$VM_PC_NAME" --storagectl "SATA Controller" --port 0 --device 0 --type hdd --medium "$VM_PC_DISK_PATH"

		# Shared Folder
		VBoxManage sharedfolder add "$VM_PC_NAME" --name "papercut_share" --hostpath "$SHARE_PATH" --automount

		echo "---------------------------------------------------"
		echo "VM '$VM_PC_NAME' is ready with OsBoxes VDI."
	}
...
configure_vm() {
	...
	#----------------------------------------------------
	is_vm_running() {
		VBoxManage list runningvms | grep -Fq "\"${VM_PC_NAME}\""
	}

	wait_for_ssh() {
        echo "Waiting for SSH to be ready on ${VM_PC_IP}..."

		sleep 2
        while true; do
            if sshpass -p "$GUEST_PASS" ssh -o StrictHostKeyChecking=no -o ConnectTimeout=2 "$GUEST_USER@${VM_PC_IP}" "echo ready" >/dev/null 2>&1; then
                echo " SSH is ready!"
                break
            else
                echo -n "."
	...
	ssh_exec_root() {
		...
		sshpass -p "$GUEST_PASS" ssh -o StrictHostKeyChecking=no \
			"$GUEST_USER@${VM_PC_IP}" "echo '$GUEST_PASS' | sudo -S -p '' bash -lc '$escaped_command'"
	}
	...
	if ! is_vm_running; then
		VBoxManage startvm "$VM_PC_NAME" --type headless
	else
		print_warn "VM '${VM_PC_NAME}' is already running; skipping startvm."
	fi
    ...
    echo "Access via: http://${VM_PC_IP}:9191/app"
    echo "Exploit Path: http://${VM_PC_IP}:9191/app?service=page/SetupCompleted"
    echo "---------------------------------------------------"
}

# Main
init
if ! VBoxManage list vms | grep -Fq "\"${VM_PC_NAME}\""; then
	provision_vm
else
	print_warn "VM '${VM_PC_NAME}' already exists; skipping provisioning."
fi
